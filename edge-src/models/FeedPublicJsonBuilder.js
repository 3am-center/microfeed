...
export default class FeedPublicJsonBuilder {
  ...
  _decorateForItem(item, baseUrl) {
    ...
    if (isValidMediaFile(item.mediaFile)) {
      item.mediaFile.isAudio =
        item.mediaFile.category === ENCLOSURE_CATEGORIES.AUDIO;
      item.mediaFile.isDocument =
        item.mediaFile.category === ENCLOSURE_CATEGORIES.DOCUMENT;
      item.mediaFile.isExternalUrl =
        item.mediaFile.category === ENCLOSURE_CATEGORIES.EXTERNAL_URL;
      item.mediaFile.isVideo =
        item.mediaFile.category === ENCLOSURE_CATEGORIES.VIDEO;
      item.mediaFile.isImage =
        item.mediaFile.category === ENCLOSURE_CATEGORIES.IMAGE;

      if (!item.mediaFile.isExternalUrl) {
        item.mediaFile.url = urlJoinWithRelative(
  ...
  _buildPublicContentChannel() {
    ...
    if (this.content.items_next_cursor && !this.forOneItem) {
      publicContent["next_url"] =
        `${publicContent["feed_url"]}?next_cursor=${this.content.items_next_cursor}&` +
    ...
    if (channel.image) {
      publicContent["icon"] = urlJoinWithRelative(
        this.publicBucketUrl,
        channel.image,
        this.baseUrl
      );
    ...
    if (this.webGlobalSettings.favicon && this.webGlobalSettings.favicon.url) {
      publicContent["favicon"] = urlJoinWithRelative(
        this.publicBucketUrl,
        this.webGlobalSettings.favicon.url,
        this.baseUrl
      );
    }
  ...
  _buildPublicContentMicrofeedExtra(publicContent) {
    ...
    if (this.content.items_prev_cursor && !this.forOneItem) {
      ...
      microfeedExtra["prev_url"] =
        `${publicContent["feed_url"]}?prev_cursor=${this.content.items_prev_cursor}&` +
        `sort=${this.content.items_sort_order}`;
    }
  ...
  _buildPublicContentItem(item, mediaFile) {
    ...
    const _microfeed = {
      ...
      rss_url: item.rssUrl,
      guid: item.guid,
      status: ITEM_STATUSES_DICT[item.status]
        ? ITEM_STATUSES_DICT[item.status].name
        : "published",
    };

    if (isValidMediaFile(mediaFile)) {
      if (mediaFile.url) {
        attachment["url"] = buildAudioUrlWithTracking(
          mediaFile.url,
          trackingUrls
        );
      }
      ...
      if (mediaFile.durationSecond) {
        ...
        _microfeed["duration_hhmmss"] = secondsToHHMMSS(
          mediaFile.durationSecond
        );
      }
  ...
  getJsonData() {
    ...
    publicContent["_microfeed"] =
      this._buildPublicContentMicrofeedExtra(publicContent);
    return publicContent;
  }
}
